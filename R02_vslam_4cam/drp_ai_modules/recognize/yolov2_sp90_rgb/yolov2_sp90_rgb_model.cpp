 ...
 ***********************************************************************************************************************/
/***********************************************************************************************************************
 * File Name    : yolov2_sp90_rgb_model.cpp
 * Version      : -
 ...
 * Includes
 ******************************************/
#include "yolov2_sp90_rgb_model.h"

YoloV2Sp90RGBModel::YoloV2Sp90RGBModel()
    : IRecognizeModel(MODEL_DIR.data(), MODEL_NAME.data(), YOLOV2_SP90_RGB_DRPAI_IN_WIDTH, YOLOV2_SP90_RGB_DRPAI_IN_HEIGHT, 3) {
    ...
    num_inf_out = (label_file_map.size() + 5) * YOLOV2_SP90_RGB_NUM_BB * num_grids[0] * num_grids[0];
 ...
 * @return int32_t
 */
int32_t YoloV2Sp90RGBModel::inf_post_process(float* arg) {
 ...
 */
shared_ptr<ObjectDetection> YoloV2Sp90RGBModel::get_object_detection() {
 ...
 */
shared_ptr<PredictNotifyBase> YoloV2Sp90RGBModel::get_command() {
 ...
 * @return int32_t
 */
int32_t YoloV2Sp90RGBModel::print_result() {
 ...
 */
void YoloV2Sp90RGBModel::post_proc(float* floatarr, std::vector<detection>& det) {
    /* Following variables are required for correct_region_boxes in Darknet implementation*/
    /* Note: This implementation refers to the "darknet detector test" */
    float new_w, new_h;
    float correct_w = 1.;
    float correct_h = 1.;
    if ((float)(YOLOV2_SP90_RGB_MODEL_IN_W / correct_w) < (float)(YOLOV2_SP90_RGB_MODEL_IN_H / correct_h)) {
        new_w = (float)YOLOV2_SP90_RGB_MODEL_IN_W;
        new_h = correct_h * YOLOV2_SP90_RGB_MODEL_IN_W / correct_w;
    }
    else {
        new_w = correct_w * YOLOV2_SP90_RGB_MODEL_IN_H / correct_h;
        new_h = YOLOV2_SP90_RGB_MODEL_IN_H;
    }

    uint32_t label_num = label_file_map.size();

    /*Post Processing Start*/
    for (uint32_t n = 0; n < YOLOV2_SP90_RGB_NUM_INF_OUT_LAYER; n++) {
        uint8_t num_grid = num_grids[n];
        uint8_t anchor_offset = 2 * YOLOV2_SP90_RGB_NUM_BB * (YOLOV2_SP90_RGB_NUM_INF_OUT_LAYER - (n + 1));

        for (uint32_t b = 0; b < YOLOV2_SP90_RGB_NUM_BB; b++) {
            for (uint32_t y = 0; y < num_grid; y++) {
                for (uint32_t x = 0; x < num_grid; x++) {
                    uint32_t offs = YoloCommon::yolo_offset(n, b, y, x, num_grids.data(), YOLOV2_SP90_RGB_NUM_BB, label_file_map.size());
                    ...
                    if (probability > YOLOV2_SP90_RGB_TH_PROB) {
                        ...
                        center_x = (center_x - (YOLOV2_SP90_RGB_MODEL_IN_W - new_w) / 2. / YOLOV2_SP90_RGB_MODEL_IN_W) / ((float)new_w / YOLOV2_SP90_RGB_MODEL_IN_W);
                        center_y = (center_y - (YOLOV2_SP90_RGB_MODEL_IN_H - new_h) / 2. / YOLOV2_SP90_RGB_MODEL_IN_H) / ((float)new_h / YOLOV2_SP90_RGB_MODEL_IN_H);
                        box_w *= (float)(YOLOV2_SP90_RGB_MODEL_IN_W / new_w);
                        box_h *= (float)(YOLOV2_SP90_RGB_MODEL_IN_H / new_h);

                        center_x = round(center_x * YOLOV2_SP90_RGB_DRPAI_IN_WIDTH);
                        center_y = round(center_y * YOLOV2_SP90_RGB_DRPAI_IN_HEIGHT);
                        box_w = round(box_w * YOLOV2_SP90_RGB_DRPAI_IN_WIDTH);
                        box_h = round(box_h * YOLOV2_SP90_RGB_DRPAI_IN_HEIGHT);
    ...
    filter_boxes_nms(det, det.size(), YOLOV2_SP90_RGB_TH_NMS);
}
