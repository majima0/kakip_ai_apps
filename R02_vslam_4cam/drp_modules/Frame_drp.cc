...
Frame_drp::Frame_drp(const int drp_fd,
                     const cv::Mat& imGray, const cv::Mat& Tcb, const double& timeStamp,
    ...
    : Frame(imGray, Tcb, timeStamp, extractor, voc, image_processing_ptr, K, distCoef, bf, thDepth, true),
...
Frame_drp::Frame_drp(const int drp_fd,
                     const cv::Mat& imGray, const cv::Mat& imDepth, const cv::Mat& Tcb, const double& timeStamp,
                     ORBextractor_drp* extractor, ORBVocabulary* voc, ImageProcessing* image_processing_ptr,
#if defined(ENABLE_DRP_AI)
                     YoloDetector* mYoloDetector,
#endif
                     cv::Mat& K, cv::Mat& distCoef, const float& bf, const float& thDepth, const int sensor_type)
    : Frame(imGray, imDepth, Tcb, timeStamp, extractor, voc, image_processing_ptr, K, distCoef, bf, thDepth, true),
      mpORBextractorLeft(extractor),
      mpORBextractorRight(static_cast<ORBextractor_drp*>(NULL)),
      drp_fd_(drp_fd),
      nlevels(mpORBextractorLeft->nlevels),
