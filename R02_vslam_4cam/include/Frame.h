...
public:
    ...
    Frame(const cv::Mat& imGray, const cv::Mat& imDepth, const cv::Mat& Tcb, const double& timeStamp, ORBextractor* extractor, ORBVocabulary* voc,
          ImageProcessing* image_processing_ptr,
          cv::Mat& K, cv::Mat& distCoef, const float& bf, const float& thDepth, const bool use_drp);

    // Constructor for Monocular cameras.
    Frame(const cv::Mat& imGray, const cv::Mat& Tcb, const double& timeStamp, ORBextractor* extractor, ORBVocabulary* voc,
          ImageProcessing* image_processing_ptr,
          cv::Mat& K, cv::Mat& distCoef, const float& bf, const float& thDepth, const bool use_drp);
...
public:
    ...
    // Camera pose in world coord.
    cv::Mat mTcw;
    // Camera pose in base coord.
    cv::Mat mTbc;
    cv::Mat mTcb;
...
private:
    ...
    std::vector<YoloBoundingBox> mvYoloBoundingBox;
};
