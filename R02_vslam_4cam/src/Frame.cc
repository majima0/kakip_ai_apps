...
Frame::Frame(const Frame& frame)
    : mvPlaneCameraCoefficients(frame.mvPlaneCameraCoefficients), mvCylinderCameraCoefficients(frame.mvCylinderCameraCoefficients),
      ...
      mTbc(frame.mTbc.clone()), mTcb(frame.mTcb.clone()),
      mTimeStamp(frame.mTimeStamp), mK(frame.mK.clone()), mDistCoef(frame.mDistCoef.clone()),
      mbf(frame.mbf), mb(frame.mb), mThDepth(frame.mThDepth), N(frame.N), mvKeys(frame.mvKeys),
      mvKeysRight(frame.mvKeysRight), mvKeysUn(frame.mvKeysUn), mvuRight(frame.mvuRight),
      mvDepth(frame.mvDepth), mBowVec(frame.mBowVec), mFeatVec(frame.mFeatVec),
      mDescriptors(frame.mDescriptors.clone()), mDescriptorsRight(frame.mDescriptorsRight.clone()),
      mvpMapPoints(frame.mvpMapPoints), mvbOutlier(frame.mvbOutlier), mnId(frame.mnId), mnCameraId(frame.mnCameraId),
...
// RGBD
Frame::Frame(const cv::Mat& imGray, const cv::Mat& imDepth, const cv::Mat& Tcb, const double& timeStamp, ORBextractor* extractor, ORBVocabulary* voc,
...
// Monocular
Frame::Frame(const cv::Mat& imGray, const cv::Mat& Tcb, const double& timeStamp, ORBextractor* extractor, ORBVocabulary* voc,
             ImageProcessing* image_processing_ptr,
             cv::Mat& K, cv::Mat& distCoef, const float& bf, const float& thDepth, const bool use_drp)
    : mpORBvocabulary(voc),
      mpORBextractorLeft(extractor), mpORBextractorRight(static_cast<ORBextractor*>(NULL)),
...
void Frame::SetYoloBoundingBoxList(const std::vector<YoloBoundingBox>& yoloBoundingBoxList) {
    ...
    //}
    mvYoloBoundingBox = yoloBoundingBoxList;
