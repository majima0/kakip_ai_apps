...
System::System(const string& strVocFile, const string& strSettingsFile,
               ...
               const std::vector<CameraInfo> camera_settings)
    ...
    mpFrameDrawer = new FrameDrawer(mpMap, camera_settings.size());
    ...
    mpTracker = new Tracking(
        ...
        mpMap, mpKeyFrameDatabase, strSettingsFile, mSensor, camera_settings.size());
    ...
    if (input_type == InputType::DATASET) {
        mpImageLoaders.push_back(new DatasetImageLoading(mSensor, image_dir_, image_filenames_, depth_filenames_, timestamp_, strSettingsFile));
        mptImageLoading.push_back(new thread(&ORB_SLAM2::ImageLoading::Run, mpImageLoaders[0]));
    }
    else if (input_type == InputType::MONO_CAMERA) {
        mpImageLoaders.push_back(new MonocularCameraImageLoading(mSensor, strSettingsFile));
        mptImageLoading.push_back(new thread(&ORB_SLAM2::ImageLoading::Run, mpImageLoaders[0]));
    }
    else if (input_type == InputType::MULTI_CAMERA || input_type == InputType::MULTI_CAMERA_CALIBRATION) {
        if(camera_settings.size() == 0) {
            cerr << "Invalid setting file: Cameras not found." << endl;
            exit(-1);
        }
        cout << "Opening " << camera_settings.size() << " cameras." << endl;
        for(int i=0; i<camera_settings.size(); ++i) {
            mpImageLoaders.push_back(new MonocularCameraImageLoading(camera_settings[i], i));
            mptImageLoading.push_back(new thread(&ORB_SLAM2::ImageLoading::Run, mpImageLoaders[i]));
        }
    }
#if defined(ENABLE_REALSENSE2)
    else if (input_type == InputType::D435i_CAMERA) {
        fprintf(stderr, "WARNING : The operation has not been verified when D435i is used.\n");
        mpImageLoaders.push_back(new D435iCameraImageLoading(mSensor, strSettingsFile));
    ...
    if(input_type == InputType::MULTI_CAMERA_CALIBRATION) {
        mpImageLoaders.front()->SeImageProcessor(mpImageProcessor);
    } else {
        for(auto it=mpImageLoaders.begin(); it!=mpImageLoaders.end(); ++it) {
            (*it)->SeImageProcessor(mpImageProcessor);
        }
    }
...
System::System(const string& strVocFile, const string& strSettingsFile,
    ...
    : System(strVocFile, strSettingsFile,
             ...
             drp_dev_num, drp_ai_dev_num, camera_settings) {
    assert(input_type == InputType::MONO_CAMERA || input_type == InputType::D435i_CAMERA || input_type == InputType::MULTI_CAMERA  || input_type == InputType::MULTI_CAMERA_CALIBRATION);
}
