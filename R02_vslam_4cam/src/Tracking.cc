...
Tracking::Tracking(
    ...
    const int drp_fd,
#endif
    System* pSys, ORBVocabulary* pVoc, FrameDrawer* pFrameDrawer, MapDrawer* pMapDrawer, Map* pMap, KeyFrameDatabase* pKFDB, const string& strSettingPath, const int sensor, const int nCameras)
    : mpORBVocabulary(pVoc), mPlaneDetector(), mPlaneMatcher(), mPlaneEffect(3),
      ...
      mpKeyFrameDB(pKFDB), mpInitializer(nCameras), mpSystem(pSys), mpViewer(NULL), mpSocketViewer(NULL),
      mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0),
      mInitialFrame(nCameras), mvIniMatches(nCameras), mvbPrevMatched(nCameras),
#if defined(ENABLE_DRP)
      drp_fd_(drp_fd)
#endif
{
...
cv::Mat Tracking::GrabImageMonocular() {
    ...
    if (mCurrentFrame->mTcw.empty())
        ...
        return mCurrentFrame->mTbc * mCurrentFrame->mTcw;
}
...
void Tracking::Track() {
    ...
    if (mState == NOT_INITIALIZED) {
        ...
        std::cerr << "Initialization start" << std::endl;
        #endif
        ...
        #if defined(ENABLE_DEBUG_OUTPUT)
        std::cerr << "Initialization " << (mState==OK ? "succeeded" : "failed") << std::endl;
        #endif
        ...
        if (bOK) {
            ...
            if (!mLastFrame.mTcw.empty()) {
                ...
                mVelocity = mCurrentFrame->mTbc * mCurrentFrame->mTcw * LastTwc * mLastFrame.mTcb;
            }
            else
                mVelocity = cv::Mat();

            mpMapDrawer->SetCurrentCameraPose(mCurrentFrame->mTbc * mCurrentFrame->mTcw);
...
void Tracking::MonocularInitialization() {
    int cameraId = mCurrentFrame->mnCameraId;

    if (!mpInitializer[cameraId]) {
        // Set Reference Frame
        if (mCurrentFrame->mvKeys.size() > 100) {
            mInitialFrame[cameraId] = Frame(*mCurrentFrame);
            mLastFrame = Frame(*mCurrentFrame);
            mvbPrevMatched[cameraId].resize(mCurrentFrame->mvKeysUn.size());
            for (size_t i = 0; i < mCurrentFrame->mvKeysUn.size(); i++)
                mvbPrevMatched[cameraId][i] = mCurrentFrame->mvKeysUn[i].pt;

            if (mpInitializer[cameraId])
                delete mpInitializer[cameraId];

            mpInitializer[cameraId] = new Initializer(*mCurrentFrame, 1.0, 200);

            fill(mvIniMatches[cameraId].begin(), mvIniMatches[cameraId].end(), -1);
        }
    } else {
        // Try to initialize
        if ((int)mCurrentFrame->mvKeys.size() <= 100) {
            delete mpInitializer[cameraId];
            mpInitializer[cameraId] = static_cast<Initializer*>(NULL);
            fill(mvIniMatches[cameraId].begin(), mvIniMatches[cameraId].end(), -1);
            return;
        }

        #if defined(ENABLE_DEBUG_OUTPUT)
        std::cerr << "Matching for initialization start" << std::endl;
        #endif
        ...
        int nmatches = matcher.SearchForInitialization(
            mInitialFrame[cameraId], *mCurrentFrame,
            mvbPrevMatched[cameraId], mvIniMatches[cameraId], 100);
        MT_FINISH(mt_search_for_initialization);

        #if defined(ENABLE_DEBUG_OUTPUT)
        std::cerr << "Matching for initialization end; nmatches=" << nmatches << std::endl;
        #endif
        ...
        if (mpInitializer[cameraId]->Initialize(*mCurrentFrame, mvIniMatches[cameraId], Rcw, tcw, mvIniP3D, vbTriangulated)) {
            for (size_t i = 0, iend = mvIniMatches[cameraId].size(); i < iend; i++) {
                if (mvIniMatches[cameraId][i] >= 0 && !vbTriangulated[i]) {
                    mvIniMatches[cameraId][i] = -1;
                    nmatches--;
                }
            }

            // Set Frame Poses
            mInitialFrame[cameraId].SetPose(cv::Mat::eye(4, 4, CV_32F));
...
void Tracking::CreateInitialMapMonocular() {
    ...
    // Create KeyFrames
    KeyFrame* pKFini = new KeyFrame(mInitialFrame[cameraId], mpMap, mpKeyFrameDB);
    ...
    for (size_t i = 0; i < mvIniMatches[cameraId].size(); i++) {
        if (mvIniMatches[cameraId][i] < 0)
            continue;
        ...
        pKFcur->AddMapPoint(pMP, mvIniMatches[cameraId][i]);

        pMP->AddObservation(pKFini, i);
        pMP->AddObservation(pKFcur, mvIniMatches[cameraId][i]);

        pMP->ComputeDistinctiveDescriptors();
        pMP->UpdateNormalAndDepth();

        // Fill Current Frame structure
        mCurrentFrame->mvpMapPoints[mvIniMatches[cameraId][i]] = pMP;
        mCurrentFrame->mvbOutlier[mvIniMatches[cameraId][i]] = false;
    ...
    mpMapDrawer->SetCurrentCameraPose(mCurrentFrame->mTbc * pKFcur->GetPose());
...
bool Tracking::TrackWithMotionModel() {
    ...
    mCurrentFrame->SetPose(mCurrentFrame->mTcb * mVelocity * mLastFrame.mTbc * mLastFrame.mTcw);
...
void Tracking::Reset() {
    ...
    for (int i=0; i<mpInitializer.size(); i++) {
        if (mpInitializer[i]) {
            delete mpInitializer[i];
            mpInitializer[i] = static_cast<Initializer*>(NULL);
        }
    }
